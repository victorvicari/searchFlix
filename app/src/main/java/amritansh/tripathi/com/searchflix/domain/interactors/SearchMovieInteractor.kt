...
class SearchMovieInteractor @Inject constructor(private val repository: Repository, private val schedulerProvider: SchedulerProvider) : SearchMovieUseCase{
    override fun searchMovie(search: String, page: Int): Single<List<Movie>> {
        return repository.search(search,page).compose(schedulerProvider.getSchedulersForSingle()).map { it : MovieResult->it.result }
    }
}